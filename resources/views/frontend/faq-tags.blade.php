                            ...
                            @foreach($tags as $tag)
                                <div class="card text-center">
                                    <div class="card-header text-left h3">
                                        <a href="/faq/{{$tag->primeTitle->slug}}/{{$tag->slug}}">{{$tag->title}}</a>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-1 dateContainer">
                                                <div class="date">
                                                    <div
                                                        class="day">{{$tag->created_at->formatLocalized('%d')}}</div>
                                                    <div
                                                        class="month">{{$tag->created_at->formatLocalized('%b')}}</div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div
                                            ...
                                            <div class="footerItems"><i class="ti-user "></i>
                                                <a href="/faq/author/{{$tag->user->slug}}-{{$tag->user->id}}"
                                                   class="specialLink"> {{$tag->user->name}}</a></div>
                                            <div class="footerItems"><i class="fa fa-tags "></i>

                                                @php($e=$tag->tags)
                                                @php($t = explode(',',$e))
                                                @foreach($t as $tt)
                                                    <a href="/faq/tags/{{\Illuminate\Support\Str::Slug($tt)}}"
                                                       class="specialLink">{{$tt}}</a>
                                                @endforeach
                                            </div>
